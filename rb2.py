...
def Reset():
    ...
    texticecream.configure(state=DISABLED)
...
def drinksicecream():
    if variable1.get() == 1:
        texticecream.configure(state=NORMAL)
        texticecream.focus()
...
def Receipt():
    ...
    textReceipt.insert(END, 'icecream:\t\t\t\t\t' + icecream.get() + '\n')
...
# Drinks
icecream = Checkbutton(Drinks_Function, text='icecream', variable=variable1, onvalue=1, offvalue=0, font=('arial', 16, 'bold'),
                    bg='sky blue', command=drinksicecream).grid(row=0, sticky=W)
...
texticecream = Entry(Drinks_Function, font=('arial', 16, 'bold'), bd=8, width=6, justify=LEFT, state=DISABLED, textvariable=icecream)
